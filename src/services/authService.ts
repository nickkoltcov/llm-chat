...
export async function exchangeCodeForKey(code:string, codeverifier:string, method:string): Promise<string> {
...
export async function provisionAppKey(userToken: string) {
    ...
    const existingKey = res.data.find((key:OpenRouterKey) => key.name === process.env.OPENROUTER_APP_KEY_NAME )

    if(existingKey) {
        const deleteKey = await fetch(`https://openrouter.ai/api/v1/keys/${existingKey.hash}`, {
            method: 'DELETE',
            headers: {
